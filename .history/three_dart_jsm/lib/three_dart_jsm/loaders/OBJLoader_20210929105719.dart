...
class ParseStateMaterial {
  ...
  late dynamic mtllib;
  late dynamic smooth;
  late dynamic groupStart;
  late dynamic groupEnd;
  late dynamic groupCount;
...
class ParseStateObject {
  ...
  _finalize( end ) {
    ...
    if ( end && this.materials.length == 0 ) {
      ...
      this.materials.add( ParseStateMaterial({
        "name": '',
        "smooth": this.smooth
      }) );
...
class ParserState {
  ...
  startObject( name, fromDeclaration ) {
    ...
    if ( previousMaterial != null && previousMaterial.name ) {
  ...
  addColor( a, b, c ) {
    ...
    if ( b != null && src[ b ] != null ) dst.push( src[ b + 0 ], src[ b + 1 ], src[ b + 2 ] );
    if ( c != null && src[ c ] != null ) dst.push( src[ c + 0 ], src[ c + 1 ], src[ c + 2 ] );
...
class OBJLoader extends Loader {
  ...
  dynamic materials;
	...
	parse( text, {String? path, Function? onLoad, Function? onError} ) {
      ...
      // print("i: ${i} line: ${line} "); 
			...
			if ( lineFirstChar == 'v' ) {
				...
				var vertexData = lineData.split( RegExp("\s+") );
				var faceVertices = [];
    ...
    print(state.objects);
    print("hasPrimitives: ${hasPrimitives} ");
        ...
        print(" createdMaterials: ${createdMaterials} ");
				...
				if ( createdMaterials.length > 1 ) {
					...
					for ( var mi = 0, miLen = materials.length; mi < miLen; mi ++ ) {
						...
						var sourceMaterial = materials[ mi ];
						buffergeometry.addGroup( sourceMaterial.groupStart, sourceMaterial.groupCount, materialIndex: mi );
        ...
        print(" container .add mesh: ${mesh} ${mesh.name} ");
